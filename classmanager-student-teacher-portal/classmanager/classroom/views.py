...
from classroom.models import StudentsInClass,StudentMarks,ClassAssignment,SubmitAssignment,Student,Teacher,VideoConference
...
def get_teacher_by_id(request,id):
...
def create_conference(request):
    ...
    room = VideoConference.objects.create(teacher=Teacher.objects.get(pk=int(request.GET.get('teacher_id'))) ,link=room_name)
    if room:
        return JsonResponse({}, status=200)
    return JsonResponse({}, status=500)

def make_video_conference(request):
    return render(request,'classroom/conference_room.html')

def join_video_conference(request):
    video_conferences = VideoConference.objects.all()
    return render(request,'classroom/join_video_conference.html',{'video_conferences':video_conferences})

def conference_room(request):
    return redirect(f"http://localhost:3000/{request.user.pk}/")

def is_link_reachable(request):
    room_name = request.GET.get('room_id')
    room = VideoConference.objects.filter(link=room_name)
    if room:
        return JsonResponse({"is_reachable":"true"}, status=200)
    return JsonResponse({"is_reachable":"false"})